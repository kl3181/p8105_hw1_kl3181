...
```
The penguins data set has 8 variables, including species, island, bill length, bill depth, flipper length, body mass, sex and study year. There are `r nrow(penguins)` rows and `r ncol(penguins)` columns of data. The mean flipper length is `r mean(penguins$flipper_length_mm)`. 
